...
$menuClass = 'nav';
    ...
    $primaryNav = wp_nav_menu( array( 'theme_location' => 'primary-menu', 'container' => '', 'fallback_cb' => '', 'menu_class' => $menuClass, 'menu_id' => 'top-menu', 'echo' => false,'walker' => new br_menuWalker() ) );

    if ( '' == $primaryNav ) :
        ?>
        <ul id="top-menu" class="<?php echo esc_attr( $menuClass ); ?>">
            <li <?php if ( is_home() ) echo( 'class="current_page_item"' ); ?>><a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Home', 'wbs' ); ?></a></li>
            <?php br_show_page_menu( $menuClass, false, false ); ?>
            <?php br_show_categories_menu( $menuClass, false ); ?>
        </ul>
    <?php
    else :
        echo( $primaryNav );
